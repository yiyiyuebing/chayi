...
Util.common = {
    //测试地址
    // b2bUrlApi: 'http://b2b.test.youchalian.com/',
    ...
    b2bPostCallback: function (url, param, callbackFun) {
        ...
        $.ajax({
            type: "POST",
            url: Util.common.b2bUrlApi+url,
            dataType: "json",
            data: param,
            headers : {
                "token": localStorage.getItem('token'),
                "X-Requested-With": "youchalian"
            },
            success: function(data){ 
                callbackFun(data);
                //token失效，跳转到登陆页
                if (!data.success && data.errorCode == '50011') {
                    new Toast({context:$('body'),message: data.msg}).show();
                    setTimeout(function() {
                        window.location.href = 'login.html?url=' + window.document.referrer;
                    }, 2000)
                }
                //店铺信息异常
                if (!data.success && data.msg == '用户信息异常，请重新登陆') {
