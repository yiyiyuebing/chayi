    ...
    createWebUploader: function(options) {
        ...
        uploader.on("error",function (type){
            if (type=="Q_TYPE_DENIED"){
                new Toast({context:$('body'),message: "请上传JPG、PNG格式文件！" }).show();
                /*layer.msg("请上传JPG、PNG格式文件");*/
            }else if(type=="F_EXCEED_SIZE"){
                new Toast({context:$('body'),message: "文件大小不能超过1M！" }).show();
               /* layer.msg("文件大小不能超过1M");*/
            }else {
                //new Toast({context:$('body'),message: "上传出错！请检查后重新上传！错误代码！" }).show();
                new Toast({context:$('body'),message: "请不要上传相同的图片" }).show();
        ...
        if (imageList.length > 0) {
            ...
            $.each(imageList, function(i, perImage) {
                var $li = $(
                        '<div id="file_' + i + '" class="file-item thumbnail" style="float:left; width:'+ thumbnailWidth +'px; height: '+ thumbnailHeight +'px;">' +
                        '<img src="'+ perImage.url +'" width="100" height="100">' +
                        '<div class="info">' + perImage.name + '</div>' +
                        '<i data-file-id="file_' + i + '" data-source="server" style="margin-top: -'+ thumbnailHeight +'px; margin-left:'+ (thumbnailWidth-14) +'px;"></i>' +
