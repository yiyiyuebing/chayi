...
<div class="content clearfix">
  <div class="content-left pull-left">
    ...
    <div class="step-waiting">
      <c:if test="${orderItemAsFlow.flowStatus == 'refuse_exchange'}">
        <h5>优茶联已拒绝申请</h5>
        ...
        <div class="waiting-operation clearfix">
          ...
          <c:if test="${goodMsgList.status == 2}">
            <c:if test="${goodMsgList.shipReturnTime < 2}">
              ...
              <a href="/afterSale/afterSaleExchange/${orderId}/${skuId}?operType=reject" class="change-server again-apply-after-sale pull-left">再次申请</a>
            </c:if>
          </c:if>
      ...
      <c:if test="${orderItemAsFlow.flowStatus == 'please_exchange'}">
        <h5>等待优茶联处理中...</h5>
        <div class="waiting-time">
          <input type="hidden" id="ipt-endTime" value="<fmt:formatDate value="${orderItemAsFlow.timeout}" pattern="yyyy-MM-dd HH:mm:ss"/>">
          优茶联在<span id="return-day">0</span>天<span id="return-hour">0</span>时<span id="return-min">0</span>分内未处理，申请将自动达成
        </div>
        <div class="waiting-operation clearfix">
          <label class="pull-left">您还可以：</label>
          <a href="javascript:void(0);" data-flow-id="${orderItemAsFlow.id}" class="cancel-server cancel-after-sale pull-left">撤销申请</a>
          <a href="/afterSale/afterSaleExchange/${orderId}/${skuId}?operType=modify" class="change-server modify-after-sale pull-left">修改申请</a>
        </div>
      </c:if>
      <c:if test="${orderItemAsFlow.flowStatus == 'exc_shipping'}">
        <h5>等待优茶联处理中...</h5>
        <div class="waiting-time">
          <%--<input type="hidden" id="ipt-endTime" value="<fmt:formatDate value="${orderItemAsFlow.timeout}" pattern="yyyy-MM-dd HH:mm:ss"/>">--%>
      ...
      <c:if test="${orderItemAsFlow.flowStatus == 'exchange'}">
        <!--同意申请模块-->
        <div class="step-agree">
          <h5>优茶联已同意申请</h5>
          <div class="waiting-time">
            <input type="hidden" id="ipt-endTime" value="<fmt:formatDate value="${orderItemAsFlow.timeout}" pattern="yyyy-MM-dd HH:mm:ss"/>">
            请在<span id="return-day">0</span>天<span id="return-hour">0</span>时<span id="return-min">0</span>分内提交换货商品的物流信息，否则系统将自动关闭您的换货申请

          </div>
          <div class="waiting-operation clearfix">
            <label class="pull-left">您还可以：</label>
            <a href="javascript:void(0);" class="see-location pull-left" id="seeLocationBtn">查看换货地址</a>
            <a href="javascript:void(0);" class="editor-logistic pull-left" id="editorLogistic">填写换货物流信息</a>
          </div>
        </div>
        <!--同意申请模块 ending-->
      </c:if>
...
<script src="/static/js/afterSale/afterSaleExchange.js"></script>
</html>