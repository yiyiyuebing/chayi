 ...
 */
var afterSaleExchange = {
    init: function() {
        afterSaleExchange.initComponent();
        afterSaleExchange.btnEvents();
    },
    initComponent: function() {
        ...
        if ($("#ipt-endTime").length > 0) {
            setInterval(afterSaleExchange.setLastTime, 0);
        }
    },
    btnEvents : function () {
        ...
        $(".submit-btn").on("click", function() {
            ...
            if ($("input[name=returnSeason]").val() && $("input[name=returnSeason]").val() != "请选择换货原因") {
                ...
                case 'modify': //修改售后
                    ...
                    $.ajax({
                        ...
                        success:function(result){
                            if (result.success) {
                                layer.msg("修改成功！");
                                window.location.href = "/afterSale/afterSaleExchange/"+ submitRefundData.orderId +"/"+ $("input[name=goodSkuId]").val() +"";
                            }else {
