          ...
          <c:if test="${!empty purchaseGoodsVos}">
            <c:forEach var="purchaseGoods" items="${purchaseGoodsVos}" varStatus="status">
              <li class="pull-left" style="position: relative;">
                ...
                && !empty purchaseGoods.promotionalInfo.bestActivity.tag1}">
                  ...
                  <c:if test="${!empty purchaseGoods.label}">
                    <i class="label-sku-${purchaseGoods.skuId}">${purchaseGoods.label}</i>
                  </c:if>
                ...
                <span class="number">销量：
                  ...
                  笔</span>
                <c:if test="${!empty purchaseGoods.promotionalInfo
                  && !empty purchaseGoods.tag2Set}">
                  <div class="promote-route clearfix">
                    <c:forEach items="${purchaseGoods.tag2Set}" var="tag2">
                        <i class="pull-left">${tag2}</i>
                    </c:forEach>
                  </div>
                </c:if>
                <div class="product-operation clearfix">
                  ...
                  <%--<c:if test="${empty purchaseGoods.promotionalInfo}">--%>
                    <c:if test="${purchaseGoods.stock > 0}">
                      <c:if test="${empty purchaseGoods.promotionalInfo || purchaseGoods.promotionalInfo.bestActivity.activityType != 'presell'}">
                        <a href="javascript:void(0);" data-id="${purchaseGoods.id}" data-sku-id="${purchaseGoods.skuId}"
                ...
                <c:if test="${!empty purchaseGoods.promotionalInfo
                && !empty purchaseGoods.promotionalInfo.activityList
                && purchaseGoods.promotionalInfo.bestActivity.activityType != 'zengpin'
                && !empty purchaseGoods.promotionalInfo.bestActivity.tag1}">
                  <div class="promotion-category">
                    <img src="${purchaseGoods.promotionalInfo.bestActivity.tag1}">
  ...
  <li class="pull-left" style="position: relative;">
    ...
    {{/if}}
      <a href="/purchase/detail/{{id}}.html" class="link-txt">
        {{if label}}
          <i class="label-sku-{{skuId}}">{{label}}</i>
        {{/if}}
        {{name}}</a>
    <span class="number">销量：
      <mark>
    ...
    {{if promotionalInfo && tag2Set}}
      <div class="promote-route clearfix">
        {{each tag2Set as tag2}}
          <i class="pull-left">{{tag2}}</i>
        {{/each}}
      </div>
    {{/if}}
      <div class="product-operation clearfix">
        ...
        {{if stock <= 0}}
          ...
          <a class="off-sale-cart pull-left hide" href="javascript:void(0);"><i></i>加入购物车</a>
        {{else}}
          {{if !promotionalInfo || promotionalInfo.bestActivity.activityType != 'presell'}}
    ...
    {{if promotionalInfo && promotionalInfo.bestActivity
    && promotionalInfo.bestActivity.activityType != 'zengpin'
    && promotionalInfo.bestActivity.tag1}}
      <div class="promotion-category">
...
<script id="tpl-img-ad" type="text/html">
  {{if adUrl}}
  <a href="{{adUrl}}" target="_blank" class="aside-link">
    <img src="{{adImgUrl}}">
  </a>
    {{else}}
  <a href="javascript:void(0)" style="pointer-events: none;" class="aside-link">
    <img src="{{adImgUrl}}">
  </>
  {{/if}}
</script>
