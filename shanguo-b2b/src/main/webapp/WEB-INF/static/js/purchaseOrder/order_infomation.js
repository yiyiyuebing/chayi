...
var orderInformation = {
     ...
     * @param shuihao
     */
    saveInvoiceInfo: function(invoiceName, content, shuihao) {
        var isSuccess = false;
        var data = {};
        data.name = invoiceName;
        data.content = content;
        data.dutyParagraph = shuihao;
        $.ajax({
            type: "POST",
            async: false,
            data: {invoiceJson: JSON.stringify(data)},
            url: "/order/saveInvoice",
    ...
    getInvoiceInfo: function() {
        $.ajax({
            ...
            success: function(result) {
                if (result.success && result.data && result.data.invoiceInfo) {
                    $("input[name=invoiceDutyParagraph]").val(result.data.invoiceInfo.dutyParagraph);
                    $("input[name=invoiceContent]").val(result.data.invoiceInfo.content);
                } else {
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    $("#ipt-"+ changeGoodNumQuery.skuId +"").attr("data-number", result.data);
                    orderInformation.getOrderInfoList();
    ...
    getOrderInfoList: function() {
        var purchaseOrder = {};
        purchaseOrder.orderType = $("input[name=orderType]").val();
        purchaseOrder.province = $("input[name=province]").val() ? $("input[name=province]").val() : "";
        purchaseOrder.city = $("input[name=city]").val() ? $("input[name=city]").val() : "";
        purchaseOrder.town = $("input[name=town]").val() ? $("input[name=town]").val() : "";
        purchaseOrder.district = $("input[name=town]").val() ? $("input[name=town]").val() : "";
        purchaseOrder.address = $("input[name=address]").val() ? $("input[name=address]").val() : "";
        ...
        purchaseOrder.orderType = $("input[name=orderType]").val() != 'presell' ? 'normal' : 'presell';
        $.ajax({
            type: "POST",
            ...
            success: function(result) {
                ...
                if (result.success) {
                    ...
                    if(result.data.baseOrder) {
                        orderInformation.main.find(".order-info").empty();
                        var orderInfo = result.data.baseOrder;
                        var isSample = null;
                        $.each(result.data.baseOrder.orderListVos, function(i, perGoods) {
                            var number = perGoods.number ? perGoods.number : 0;
                            var price = perGoods.purGoodsAmount ? Number(perGoods.purGoodsAmount) : 0;
                            perGoods.price = price;
                            if (perGoods.isSample) {
                                isSample = 1;
                            }

                            if (!perGoods.purGoodsSkuId) {
                                perGoods.purGoodsSkuId = "";
                            }

                            perGoods.totalPrice = perGoods.finalAmount;
                            var tpl_product_info = template("tpl-product-info", perGoods);
                            orderInformation.main.find(".order-info").append($(tpl_product_info));
                        });

                        var carriage = orderInfo.carriage ? Number(orderInfo.carriage) : 0.00;
                        var totalPrice = orderInfo.totalPrice ? Number(orderInfo.totalPrice) : 0.00;
                        var discountAmount = orderInfo.discountAmount ? Number(orderInfo.discountAmount) : 0.00;
                        var finalAmount = orderInfo.finalAmount ? Number(orderInfo.finalAmount) : 0.00;



                        orderInformation.main.find(".order-carrige-price").html("¥" + carriage.toFixed(2)); //设置总价
                        orderInformation.main.find(".order-total-price").html("¥" + totalPrice.toFixed(2)); //设置总价
                        $(".order-footer-cost").find(".discount-amount").html("-¥" + discountAmount.toFixed(2)); //设置总价
                        $(".order-footer-cost").find(".total-payment").html("¥" + finalAmount.toFixed(2)); //设置总价
                    ...
                    if (result.data.freightResult && result.data.freightResult.goodFreights) {
                        var  freightTplInfo = result.data.freightResult.servicerList[0];
                        var html = "";
                        ...
                        orderInformation.main.find(".order-carrige-price").html("¥" + freightTplInfo.totalFreight.toFixed(2)); //设置总价
            ...
            success: function(result) {
                if (result.success && result.data.graList) {
                    ...
                    $.each(result.data.graList, function(i, perAddr) {
                        ...
                        if (perAddr.status == 1) {
                            $("input[name=province]").val(perAddr.provinceName);
                            $("input[name=city]").val(perAddr.cityName);
                            $("input[name=town]").val(perAddr.areaName);
                            $("input[name=address]").val(perAddr.detailAddr);
                        }
    ...
    selectAddress:function() {
        orderInformation.main.find(".order-address-group").find(".input-mask").click(function(){
            ...
            orderInformation.isDefaultAddress(orderInformation.main.find(".order-address-group"));
            orderInformation.getReceiveAddress();
            orderInformation.getOrderInfoList(); //计算费用合计
        });
    },
    ...
    calTotalPrice: function() {
        ...
        orderInformation.main.find(".product-count").find("input[name=number]").each(function(i, perInput) {
            ...
            discountAmount += $(perInput).attr("data-discount-amount") ? Number($(perInput).attr("data-discount-amount")) : 0;
            finalAmount += price * number;
        });
        totalPrice = ((finalAmount - discountAmount)  + carriage).toFixed(2); //计算总价
        orderInformation.main.find(".order-carrige-price").html("¥" + carriage.toFixed(2)); //设置总价
        orderInformation.main.find(".order-total-price").html("¥" + finalAmount.toFixed(2)); //设置总价
        $(".order-footer-cost").find(".discount-amount").html("-¥" + discountAmount.toFixed(2)); //设置总价
    ...
    submitOrderInfo: function() {
        ...
        $(".order-info").find("input[name=number]").each(function(i, perInput) {
            var orderListInfo = {};
            if (parseInt($(perInput).val()) != 0 && $(perInput).attr("data-good-type") != "zengpin") {
                orderListInfo.number = $(perInput).val();
                orderListInfo.purGoodsSkuId = $(perInput).attr("data-sku-id");
    ...
    selectedBill: function(ev) {
        ...
        var isCompany = $(".bill-change-category").find("input[name=invoiceName]:checked").attr("data-value") == "company" ? true : false;

        if (isCompany) {
            var invoiceName = invoiceType;
            var content = $("input[name=invoiceContent]").val();
            var dutyParagraph = $("input[name=invoiceDutyParagraph]").val();
            var isSccuss = orderInformation.saveInvoiceInfo(invoiceName, content, dutyParagraph);
            if (isSccuss) {
    ...
    billCategory:function(){
        $(".bill-change-category-group").click(function(ev){
            ...
            if (invoiceName == "company") {
                $(".invoice-company-css").removeClass("hide");
                $(".bill-change-wrap").css({"height": "300px"});
                orderInformation.getInvoiceInfo();
            } else {
                if (!$(".invoice-company-css").hasClass("hide")) {
                    $(".invoice-company-css").addClass("hide");
                }
