...
var  PurchaseGoodsDetail = {
    init:function(){
        ...
        if (activityEnd) {
            activityEndDate = new Date(activityEnd).getTime();
            setInterval(PurchaseGoodsDetail.countDownTime, 0);
        }
        if (activityStart) {
            activityStartDate = new Date(activityStart).getTime();
        ...
        PurchaseGoodsDetail.favourSlide();
    ...
    countDownTime: function() {
        ...
        $(".activity-end-time").html(d +"天"+ h +"时"+ m +"分"+ s +"秒");
    },
    ...
    countDownTime1: function() {
        var EndTime = new Date(activityStart);
        var NowTime = new Date();
        var t =EndTime.getTime() - NowTime.getTime();
        var d=0;
        var h=0;
        ...
        $(".activity-start-time").html(d +"天"+ h +"时"+ m +"分"+ s +"秒");
    },
    ...
    createPreOrder: function() {
        ...
        data.orderType = $("input[name=orderType]").val();
        $.ajax({
            type: "POST",
    ...
    /*优惠政策*/
    favourSlide:function(){
        var rowHeight = $("#favourList").find("li").eq(0).outerHeight(true);
        $("#favourList").css({"height":rowHeight});
        $("#iconSlide").on("click",function(){
            if($(this).hasClass("down")){
                $(this).siblings(".favour-list").css({"height":'auto'});
                $(this).removeClass("down");
                $(this).addClass("up");
            }else{
                $(this).siblings(".favour-list").css({"height":'25px'});
                $(this).removeClass("up");
                $(this).addClass("down");
            }
        })
