...
// if current browser is not support sessionStorage
...
function QiniuJsSDK() {
    ...
    this.uploader = function(op) {
        ...
        uploader.bind('BeforeUpload', function(up, file) {
            ...
            if ((uploader.runtime === 'html5' || uploader.runtime === 'flash') && chunk_size) {
                if (file.size < chunk_size || is_android_weixin_or_qq()) {
                    ...
                    // ISSUE: if file.name is existed in sessionStorage 
                    // but not the same file maybe cause error
                    var sessionFileInfo = sessionStorage.getItem(file.name);
                    ...
                    if (sessionFileInfo) {
                        ...
                        sessionFileInfo = that.parseJSON(sessionFileInfo);
                        var now = (new Date()).getTime();
                        var before = sessionFileInfo.time || 0;
                        ...
                        if (now - before < aDay) {
                            ...
                            if (sessionFileInfo.percent !== 100) {
                                if (file.size === sessionFileInfo.total) {
                                    // TODO: if file.name and file.size is the same 
                                    // but not the same file will cause error
                                    file.percent = sessionFileInfo.percent;
                                    file.loaded = sessionFileInfo.offset;
                                    ctx = sessionFileInfo.ctx;

                                    // set speed info
                                    speedCalInfo.isResumeUpload = true;
                                    speedCalInfo.resumeFilesize = sessionFileInfo.offset;

                                    // set block size
                                    if (sessionFileInfo.offset + blockSize > file.size) {
                                        blockSize = file.size - sessionFileInfo.offset;
                                    }
                                } else {
        ...
        uploader.bind('ChunkUploaded', function(up, file, info) {
            ...
            sessionStorage.setItem(file.name, that.stringifyJSON({
                ctx: ctx,
