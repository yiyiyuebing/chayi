...
type, read_num as readNum, author, video_url as videoUrl, study_tag as studyTag, file, cove_pic as covePic,
...
create_by as createBy, update_time as updateTime, update_by as updateBy, content
from event_online_study  where 1=1 AND video_show=0 and is_valid="T"
<#if title?? && label?? && title?length gt 0 && label?length gt 0>
    AND (title like '%${title}%' OR label like '%${label}%' )
</#if>
<#if studyType??  && studyType?length gt 0>
    and study_type = ${studyType}
</#if>
<#if studyChildType?? && studyChildType?length gt 0>
...
order by ${order}
<#if orderType?? && orderType?length gt 0>
    ${orderType}
</#if>
