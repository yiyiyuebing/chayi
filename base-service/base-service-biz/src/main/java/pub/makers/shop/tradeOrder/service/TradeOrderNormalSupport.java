...
@Service
public class TradeOrderNormalSupport extends BaseOrderSupportImpl {
...
//    private AJpushUtils aJpushUtils;
	...
	protected void doCreateOrder(final BaseOrder bo) {
        ...
        transationTemplate.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(TransactionStatus status) {
		        ...
		        System.out.println(String.format("请求%s,锁定库存开始", requestId));
				// 锁定库存
		    	stockBizService.lockStocks(bo.getItemList(), getOrderBizType());
		    	System.out.println(String.format("请求%s,锁定库存完成，耗时%dms", requestId, startTime - new Date().getTime()));
		    	System.out.println(String.format("请求%s,保存订单开始", requestId));
				// 保存订单信息
		    	indentService.saveOrder(indentVo);
		    	System.out.println(String.format("请求%s,保存订单结束，耗时%dms", requestId, startTime - new Date().getTime()));
		        // 新增出库单
		    	System.out.println(String.format("请求%s,新增出库单开始", requestId));
		        cargoOutboundBizService.addOutbound(indentVo.getId(), getOrderBizType(), Long.valueOf(indentVo.getBuyerId()), "销售出库");
		        System.out.println(String.format("请求%s,新增出库单结束，耗时%dms", requestId, startTime - new Date().getTime()));
			}
		});
	...
	protected void doCancelOrder(BaseOrder bo, OrderCancelType cancelType) {
		...
		// 更新库存
        stockBizService.releaseLockStocks(indentList, getOrderBizType());
        // 取消出货单
    ...
    public void doPayOrder(final OrderPayInfo payInfo, BaseOrder bo) {
        ...
        List<IndentList> indentList = indentListService.listByOrderId(bo.getOrderId());
    	stockBizService.useLockStocks(indentList, getOrderBizType());

//        aJpushUtils.sendByAlias(indent.getSubbranchId() + "","您有一笔订单，请注意查看！", PushMsgType.order.name(), indent.getId() + "");
    ...
    public void shipmentOrder(final ShippingInfo si) {
        ...
        transcationTemplate.execute(new TransactionCallbackWithoutResult() {
            ...
            protected void doInTransactionWithoutResult(TransactionStatus transactionStatus) {
                ...
                si.setExpressId(order.getExpressId());
                System.out.println(new Date());
                // 更新订单状态
            	indentService.updateOrderToShipment(order, si);
                System.out.println(new Date());
            	stockBizService.useStocks(order.getItemList(), getOrderBizType());
                System.out.println(new Date());
            }
        });
