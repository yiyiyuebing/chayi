...
<#else>
  SELECT * FROM (
      SELECT pgs.id,pg.pur_goods_name AS NAME,pg.group_id,cs.retail_price,
...
<#else>
  SELECT * FROM (
      SELECT pgs.id,pg.name AS NAME,pg.group_id,cs.retail_price,
        ( SELECT i.pic_url FROM image i WHERE i.group_id = c.pc_album_id LIMIT 1) AS smallImage,
        pgs.good_id AS goodsId,
        cs. CODE,
        "T" as isRelevance
      FROM
        trade_good_sku pgs
      LEFT JOIN cargo_sku cs ON cs.id = pgs.cargo_sku_id
      LEFT JOIN cargo c ON cs.cargo_id = c.id
      LEFT JOIN trade_good pg ON pg.id = pgs.good_id
      WHERE
        1 = 1
      AND pgs.`status` = '1'
      <#if classifyName?? && classifyName!="-1" && classifyName!="">
