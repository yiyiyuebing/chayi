select
 i.id, i.name, i.receiver, i.receiver_phone as receiverPhone, concat(IFNULL(i.province,''), IFNULL(i.city,''), IFNULL(i.town,''), IFNULL(i.address,'')) as address,
 ...
 i.indent_shop_type,
 (case when tolp.pay_status = 'T' then tolp.id else '' end)  as weixinTraderId
from
indent i
...
where 1=1 and i.status != 0
      <#if ids??>
         and i.id in (${ids})
      <#else>
      <#if weixinTraderId?? && weixinTraderId?length gt 0> AND tolp.id LIKE concat('%', '${weixinTraderId ! ''}', '%')</#if>
      <#if concatName?? && concatName?length gt 0> and r.concat_name LIKE concat('%', '${concatName ! ''}', '%') </#if>
		 ...
		 <#if storeName?? && storeName?length gt 0> and shopmain.name like concat('%', '${storeName ! ''}', '%') </#if>
		  <#if storeNum?? && storeNum?length gt 0> and shopmain.number = ${storeNum} </#if>
		  <#if storeDeptNum?? && storeDeptNum?length gt 0> and shopmain.department_num = ${storeDeptNum} </#if>
		 ...
		 <#if receiverPhone?? && receiverPhone?length gt 0> and i.receiver_phone like concat('%', '${receiverPhone ! ''}', '%') </#if>
		  ...
		  <#if province?? && province?length gt 0> and shopmain.province_name like concat('%', '${province ! ''}', '%')  </#if>
		  <#if city?? && city?length gt 0> and shopmain.city_name like concat('%', '${city ! ''}', '%')  </#if>
		  <#if country?? && country?length gt 0> and shopmain.country_name like concat('%', '${country ! ''}', '%') </#if>
      </#if>
